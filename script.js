...
const swtchLang = (Ctrl = true, AltLeft = true) => {
  if (Ctrl && AltLeft && currentLanguage === 'RU') {
    allBtns.forEach((item) => {
      const btn = item;
      allKeys.forEach((key) => {
        if (item.classList.contains(key.code) && (!item.classList.contains('service-key') || key.code === 'Lang')) {
          btn.textContent = isLowerCase ? key.key.en : key.key.en.toUpperCase();
        }
      });
    });
    currentLanguage = 'EN';
  } else if (Ctrl && AltLeft && currentLanguage === 'EN') {
    allBtns.forEach((item) => {
      const btn = item;
      allKeys.forEach((key) => {
        if (item.classList.contains(key.code) && (!item.classList.contains('service-key') || key.code === 'Lang')) {
          btn.textContent = isLowerCase ? key.key.ru : key.key.ru.toUpperCase();
        }
      });
    });
    currentLanguage = 'RU';
